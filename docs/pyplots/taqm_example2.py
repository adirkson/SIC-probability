...
# Change to `True` if you would like to see how calibration is
# performed when case 2 is encountered
special_case = False
...
# Get TAMH from MH
pval_x = linregress(tau_t,X.mean(axis=1))[3]  #first check MH trend over tau_t                  
if pval_x<0.05:
...
# Get TAOH from OH   
pval_y = linregress(tau_t,Y)[3]     #first check OH trend over tau_t                  
if pval_y<0.05:   
